...
The cv.json is cached in Local Storage. The data in Local Storage has been timestamped and 'automatically' invalidated after 24h, so that the cache eventually reflects the cv.json on the server when the cv.json is updated on the server.
