...
The cv.json is cached in Local Storage. The data in Local Storage is timestamped and 'automatically' invalidated after 24h, so that the cache eventually reflects the cv.json on the server, if the cv.json is updated on the server.
